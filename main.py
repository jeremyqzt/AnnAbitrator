...
csv_train = pd.read_csv(
    "./data/amount.csv",
    names=["Description", "Input", "Status", "Type", "Amount", "ShipAmount", "TaxAmount",
           "TotalAmount", "Success"],
    encoding="utf-8",
...
to_train = []
for vec in in_vec:
    to_convert = make_input_vector([{"size": 7, "to_set": vec[0]}, {
...
combinedInput = concatenate([pos.output, neg.output, other.output])
...
if create_model:
    ...
    model = Model(inputs=[pos.input, neg.input, other.input], outputs=x)
